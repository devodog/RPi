 ...
 * @author Dkv
 * @brief
 * This is the main file for the Raspberry Pi Pico 2 LED-Strip Driver with dimmer.
 * This code is designed to control the brightness of two LED strips using PWM signals.
 * The brightness is controlled by two potentiometers connected to the ADC inputs of the Raspberry Pi Pico 2.
 * The code also includes a temperature sensor (LM35) to monitor the temperature of the device.
 * If the temperature exceeds a certain limit, the code will reduce the PWM duty cycle to prevent overheating.
 * If the device is overheated for too long, the program will shut down.
 * The code also includes a soft start feature to gradually increase the brightness of the LED strips when the potentiometer is turned on.
 * 
 * @version 1.0
 * Initial Date: 05FEB25
 * 
 * As of to day 6. MAY 2025 th code supports the LEDstripDrv2.0 hardware 
 * that provides two individual PWM channels for controlling the brightness 
 * of two LED strips. The code uses the ADC to read the potentiometer values 
 * and adjusts the PWM duty cycle accordingly. 
 * 
 * The code also includes a temperature sensor (LM35) to monitor the 
 * temperature of the device (the sensor is lose to Q1, one of the MOSFET 
 * transistors). If the temperature exceeds a certain limit, the code will 
 * reduce the PWM duty cycle to prevent overheating. 
 * 
 * If the overheating last for too long the device shuts down. 
 * The code also includes a soft start feature to gradually increase the 
 * brightness of the LED strips when the potentiometer is turned on.
 * 
 * 
 * 
 * DEVELOPMENT HISTORY:
 ...
 *   and wait for the temperature to drop below 30 degrees Celsius before resuming normal operation.
 *   If the device is overheated for too long, the program will shut down.
 * 
 */
#include <stdio.h>
